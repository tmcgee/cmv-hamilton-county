...
define([
  ...
  'dojo/dom-style',
  'dojo/dom-attr',
  'dojo/topic',  //lcs to support User Preferences
  'dijit/_Widget',
  'dijit/_Templated',
  'dojo/Evented',
  'dojox/gfx',
...
], function(declare, has, aspect, lang, array, domConstruct, domClass, domStyle, domAttr, topic, _Widget, _Templated, Evented, gfx, coreFx, Toggler, jsonUtils, scaleUtils, esriConfig, ArcGISDynamicMapServiceLayer, ArcGISTiledMapServiceLayer, sniff, template) {
  ...
  var _TOCNode = declare([_Widget, _Templated], {
        ...
        require(["dijit/form/HorizontalSlider", "dojo/domReady!"], function(HorizontalSlider) {
          me.slider = new HorizontalSlider({
            ...
            onChange: function(value) {
              ...
              if (rootLayer.visible) { me._startPublishTimer(); }  //lcs to support User Preferences
            },
            layoutAlign: 'right'
          });
    ...
    _onClick: function(evt) {
      ...
      if (t == this.checkNode || dijit.getEnclosingWidget(t) == this.checkNode) {
        ...
        this._startPublishTimer();  //lcs to support User Preferences
    ...
    // lcs function to support User Preferences
    _startPublishTimer: function() {
        if (this._publishTimer) {
            clearInterval(this._publishTimer);
        }
        this._publishTimer = setTimeout(lang.hitch(this, '_publishOpLayersChanged'), 1000);
    },
    // lcs function to support User Preferences
    _publishOpLayersChanged: function() {
        if (this._publishTimer) { 
            clearInterval(this._publishTimer); 
            this._publishTimer = null;
        }
        topic.publish('USER_PREFERENCES', { restoreMapLayers: { version: '$v3', operational: [] } });
    }
  });
